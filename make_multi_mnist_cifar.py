...
if sys.argv[1] == 'training':
    set = tv.datasets.CIFAR10(root='./data/cifar/', train=True, download=True, transform=transform)
    train_set = ConcatDataset([set, set, set])
    loader = torch.utils.data.DataLoader(train_set, batch_size=1, shuffle=False, num_workers=0, drop_last=False)
    len_cifar = len(loader)
    print(len_cifar)
    dataiter = iter(loader)

if sys.argv[1] == 'test':
    set = tv.datasets.CIFAR10(root='./data/cifar/', train=False, download=True, transform=transform)
    loader = torch.utils.data.DataLoader(set, batch_size=1, shuffle=False, num_workers=0, drop_last=False)
    len_cifar = len(loader)
    print(len_cifar)
    dataiter = iter(loader)
...
def multi_mnist(save_dir, num_images=60000, kind='training', max_overlap=.1, plot=False, add_distractor=False, nb_objects=2):
    ...
    for image in range(num_images):
        ...
        cifar_imgs, cifar_lbls = next(dataiter)
        new_cifar = cifar_imgs.squeeze() #tv.transforms.functional.rgb_to_grayscale(cifar_imgs.squeeze())
        print(torch.max(new_image), torch.min(new_image))
        new_image = new_cifar + new_image/255
        new_image = torch.where(new_image > 1, torch.ones_like(new_image), new_image)


        mask = torch.nn.functional.interpolate(mask.unsqueeze(0).unsqueeze(0),
                                                    scale_factor=32 / im_size,
                                                    mode='nearest').squeeze() #-exact
...
if __name__ == '__main__':
    ...
    save_dir = './data/multi_mnist_cifar2/multi_mnist_cifar/{}'.format(kind)  # ,max_overlap,num_digits+1
